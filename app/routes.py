...
from app.queries.dashboard import (
    get_attachment_count,
    get_email_campaigns_count,
    get_email_processed_per_day,
    get_ips_count,
...
from app.queries.email_details import (
    get_attachments_for_email,
    get_email_details,
    get_recipients_for_email,
    get_urls_for_email,
)
...
def campaigns():
    ...
    return render_template(
        "campaigns.html", records=results, page=page, size=size, total_pages=total_pages
    )
...
def email_details(campaign_id, mail_id):
    ...
    if not record:
        pass
    recipients = get_recipients_for_email(mail_id)
    attachments = get_attachments_for_email(mail_id)
    urls = get_urls_for_email(mail_id)
    record["unique_recipients"] = recipients["unique_recipients"] or []

    for _tmp_attachment in attachments:
        for _attachments_details in _tmp_attachment.get("attachments_details") or []:
            if not _attachments_details:
                continue
            if _attachments_details.get("result"):
                _attachments_details["total_av_engines"] = sum(
                    _attachments_details["result"]["analysis_stats"].values()
                )

                _attachments_details["malicious_found"] = (
                    _attachments_details["result"]["analysis_stats"].get("malicious")
                    or 0
                )
    record["attachments_details"] = attachments or []
    record["urls_details"] = urls["urls_details"] or []
